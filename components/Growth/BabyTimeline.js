...
const BabyTimeline = ({ 
  ...
  const handleDayPress = (day) => {
    ...
    // If day has an entry, navigate to detail screen
    if (day.hasEntry && day.entry) {
      navigation.navigate('EntryDetailScreen', {
        entry: day.entry,
        date: day.dateStr, // Pass date string for formatting
      });
      return;
    }
    
    // Otherwise, call parent's onDayPress (which might open the AddEntryModal)
      ...
      <View style={styles.dailySummary}>
        ...
        {hasTodayEntry ? (
          <View style={styles.todayEntryPreview}>
            ...
            {/* View Details button - Navigate to EntryDetailScreen */}
            <TouchableOpacity 
              style={styles.viewDetailsButton}
              onPress={() => {
                if (todayData) {
                  // Direct navigation to EntryDetailScreen
                  navigation.navigate('EntryDetailScreen', {
                    entry: todayEntry || todayData.entry,
                    date: todayStr,
                  });
                }
              }}
          ...
          <View style={styles.noEntryContainer}> 
            {/* No entry content remains the same */}
          </View>
        )}
      </View>
    );
