...
const AddEntryModal = ({ visible, day, onClose, onSave, existingEntry }) => {
  ...
  // Function to format the date for display in the modal title
  const getFormattedDate = () => {
    if (!day) return '';
    
    // Check if day has format method (it's likely a moment.js object)
    if (typeof day.format === 'function') {
    ...
    // If day is a string in ISO format
  ...
  const handleSave = () => {
    ...
    // Create a title for the entry based on entry types
    let entryTitle = "Baby's Day";
    if (entryData.entryTypes.includes('milestone') && entryData.milestones.length > 0) {
      entryTitle = `Milestone: ${entryData.milestones[0]}`;
    } else if (entryData.entryTypes.includes('growth')) {
      entryTitle = "Growth Update";
    } else if (entryData.entryTypes.includes('media') && entryData.mediaItems.length > 0) {
      entryTitle = entryData.mediaItems[0].type === 'image' ? "Photo Day" : 
                 entryData.mediaItems[0].type === 'audio' ? "Audio Recording" : "Document Added";
    }
    
    // Add a timestamp to the entry data
    const timestamp = new Date().toISOString();
    ...
    const completeEntryData = {
      ...
      title: entryTitle,
      timestamp,
      date: day ? (typeof day.format === 'function' ? day.format('YYYY-MM-DD') : 
                 day instanceof Date ? day.toISOString().split('T')[0] : 
                 typeof day === 'string' ? day : 
                 new Date().toISOString().split('T')[0]) : 
             new Date().toISOString().split('T')[0]
    };
    
    onSave(day, completeEntryData);
    onClose();
  };
