...
export default function Register({ navigation }) {
  ...
  const handleRegister = async () => {
    if (validateForm()) {
      ...
      try {
        ...
        // Create user data object to pass to next screen
        const userData = {
          fullName,
          email
        };
        
        // Navigate to Birthday screen on success
        navigation.navigate('Birthday', { userData });
      } catch (error) {
        Alert.alert(
          "Registration Failed",
  ...
  const handleSocialLoginSuccess = (socialData) => {
    // socialData would contain user information returned from Google/Apple login
    // Navigate to Birthday screen with user data from social login
    navigation.navigate('Birthday', { userData: socialData });
  };
